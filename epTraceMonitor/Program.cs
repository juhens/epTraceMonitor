...
namespace epTraceMonitor
{
    ...
    class epTraceMonitor
    {
        ...
        public epTraceMonitor(string[] args)
        {
            ...
            IntPtr modeHandle = GetStdHandle(-10);
            UInt32 consoleMode;
            GetConsoleMode(modeHandle, out consoleMode);

            consoleMode &= ~((uint)0x0040);
            SetConsoleMode(modeHandle, consoleMode);
        ...
        private void OpenProcess()
        {
            ...
            try
            {
                j = new Core.JhMemory(Process.GetProcessesByName("StarCraft")[0]);
            }
            catch
            {
        ...
        //init(w), main(r)
        private Core.JhMemory j = new(Process.GetCurrentProcess());
